...
class ExP():
    ...
    def get_source_data(self):
        # Modified LOSO: 
        # - Training data: T data from all subjects except nSub (sub2-9 if nSub=1)
        # - Test data: E data from all subjects except nSub (sub2-9 if nSub=1)

        train_data_list = []
        train_label_list = []

        # Load training data: T data from all subjects except the test subject
        for subject_id in range(1, self.total_sub + 1):
            if subject_id == self.nSub:
                continue  # Skip the test subject

            # Load only T data for training
        ...
        for subject_id in range(1, self.total_sub + 1):
            ...
            # Load only E data for testing
            file_path = self.root + 'A0%dE.mat' % subject_id
            print(f'Loading test data (E): {file_path}')
    ...
    def train(self):
        ...
        try:
            ...
            excel_path = f'./results/test_results_subject{self.nSub}.xlsx'
            results_df.to_excel(excel_path, index=False, engine='openpyxl')
            print(f'测试结果已保存到: {excel_path}')
            print(f'测试样本数: {len(results_df)}')
            
            # 计算最终测试准确率
            final_test_acc = float((test_predictions == test_label).cpu().numpy().astype(int).sum()) / float(test_label.size(0))
            print(f'最终测试准确率: {final_test_acc:.4f} ({final_test_acc*100:.2f}%)')
