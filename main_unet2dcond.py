...
def main():
    ...
    print(f"类别数量: {num_classes}, 条件嵌入维度: {cond_emb_dim}")
    ...
    # 获取条件嵌入（在 CPU 上测试）
    with torch.no_grad():
        cond_emb = label_embedder(sample_label)  # (1, cond_emb_dim)
        cond_emb = cond_emb.unsqueeze(1)  # (1, 1, cond_emb_dim) - UNet 需要序列维度
...
def train_loop_conditional(
    ...
    # 准备模型和数据加载器
    # 注意：需要将 unet 和 label_embedder 组合成一个模型，或者分别准备
    # 为了简化，我们将它们组合成一个包装类
    class ConditionalModel(torch.nn.Module):
        ...
        def forward(self, noisy_images, timesteps, labels):
            # 确保标签在正确的设备上
            labels = labels.to(noisy_images.device)

            # 获取条件嵌入
            cond_emb = self.label_embedder(labels)  # (B, cond_emb_dim)
            # 需要扩展 cond_emb 的维度以匹配 UNet 的期望输入
            # UNet2DConditionModel 期望 encoder_hidden_states 的形状为 (B, seq_len, cond_emb_dim)
            # 我们需要添加一个序列长度维度
            cond_emb = cond_emb.unsqueeze(1).repeat(1, 4, 1)  # (B, 4, cond_emb_dim)

            # 确保条件嵌入在正确的设备上
            cond_emb = cond_emb.to(noisy_images.device)
    ...
    for epoch in range(config.num_epochs):
        ...
        if accelerator.is_main_process and ((epoch + 1) % config.save_image_epochs == 0 or epoch == config.num_epochs - 1):
            ...
            unwrapped_label_embedder = accelerator.unwrap_model(model).label_embedder
...
def generate_conditional_samples(unet, label_embedder, noise_scheduler, config, epoch, pic_dir, num_classes, device):
    ...
    with torch.no_grad():
        for class_id in range(num_classes):
            ...
            # 获取条件嵌入
            cond_emb = label_embedder(labels)  # (B, cond_emb_dim)
            cond_emb = cond_emb.unsqueeze(1)  # (B, 1, cond_emb_dim)
